...
public interface AdminUserManageApi {
    ...
    ResponseEntity<Void> approveUser(
            @Parameter(description = "승인할 회원의 ID") @PathVariable Long userId
    );
    ...
    ResponseEntity<Void> rejectUser(
            @Parameter(description = "거절할 회원의 ID") @PathVariable Long userId
    );
    ...
    ResponseEntity<UserInfoPageResponseDto> getAllUsers(
            @Parameter(description = "페이지 번호 (0부터 시작)") @RequestParam(defaultValue = DEFAULT_PAGE) int page,
            @Parameter(description = "페이지 당 항목 수") @RequestParam(defaultValue = DEFAULT_SIZE) int size,
            @Parameter(description = "정렬 기준 필드명") @RequestParam(defaultValue = DEFAULT_SORT_BY) String sortBy,
            @Parameter(description = "정렬 순서 (ASC 또는 DESC)") @RequestParam(defaultValue = DEFAULT_ORDER) SortOrder order
    );
}
